...
- [ ] CDN dependencies (Tailwind, esm.sh) should be bundled locally for true offline
- [ ] App.tsx manual song mapping should use spread or a shared transformer
- [ ] TrainingPanel has hardcoded English strings (Section titles, placeholders)
- [ ] Progress bar for generation (predict() is blocking in Gradio)
- [ ] Stop generation button not implemented
- [ ] Hot LM model switching not implemented in `init_service_wrapper`
